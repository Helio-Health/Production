  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-zinc-950 text-white flex items-center justify-center">
        <div className="text-zinc-500 text-sm">Loading your dashboard...</div>
      </main>
    )
  }
    ...
    <main className="min-h-screen bg-zinc-950 text-white flex flex-col">
      ...
      <nav className="px-8 py-6 flex items-center justify-between max-w-5xl mx-auto w-full border-b border-white/10">
        ...
        <div className="flex items-center gap-4">
          <Link href={`/doctors/${doctor.id}`} className="text-sm text-zinc-400 hover:text-white transition-colors" target="_blank">
            View my profile
          </Link>
      ...
      <div className="max-w-5xl mx-auto w-full px-6 py-10 flex flex-col gap-6">
        ...
        <div className="bg-zinc-900 border border-white/10 rounded-3xl p-6 flex items-center justify-between">
          <div>
            <div className="font-semibold text-white mb-1">Accepting new patients</div>
          ...
          <div className="flex items-center gap-3">
            <span className={`text-sm font-medium ${doctor.accepting_patients ? 'text-emerald-400' : 'text-zinc-500'}`}>
              {doctor.accepting_patients ? 'Active' : 'Paused'}
            </span>
            <button onClick={toggleAccepting} disabled={toggling} className={`w-12 h-6 rounded-full transition-all relative ${doctor.accepting_patients ? 'bg-emerald-500' : 'bg-zinc-700'}`}>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-zinc-900 border border-white/10 rounded-2xl p-5">
            <div className="text-xs text-zinc-500 mb-2">Total requests</div>
          ...
          <div className="bg-zinc-900 border border-white/10 rounded-2xl p-5">
            <div className="text-xs text-zinc-500 mb-2">Accepted</div>
            <div className="text-3xl font-bold text-emerald-400">{accepted.length}</div>
          </div>
        </div>
        ...
        <div className="bg-zinc-900 border border-white/10 rounded-3xl p-6">
          ...
          {matches.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-zinc-500 text-sm">No match requests yet.</p>
              <p className="text-zinc-600 text-xs mt-1">Share your profile to start receiving requests.</p>
            </div>
          ) : (
            <div className="flex flex-col gap-4">
              {matches.map(match => (
                <div key={match.id} className="flex items-start gap-4 p-4 bg-zinc-950 rounded-2xl border border-white/5">
                  <div className="w-10 h-10 rounded-full bg-zinc-800 flex items-center justify-center text-zinc-400 font-medium text-sm flex-shrink-0">
                    {match.patient_name ? match.patient_name.split(' ').map((n: string) => n[0]).join('') : '?'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-white text-sm">{match.patient_name || 'Anonymous patient'}</div>
                    <div className="text-xs text-zinc-500 mb-2">{match.patient_email}</div>
                    {match.message && <p className="text-sm text-zinc-400 italic">"{match.message}"</p>}
                  </div>
                  <div className="flex-shrink-0">
                    {match.status === 'pending' ? (
                      <div className="flex gap-2">
                        <button onClick={() => respondToMatch(match.id, 'accepted')} className="px-3 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-black text-xs font-semibold rounded-xl transition-all">Accept</button>
                        <button onClick={() => respondToMatch(match.id, 'declined')} className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-400 text-xs font-medium rounded-xl transition-all">Decline</button>
                      </div>
                    ) : (
                      <span className={`text-xs font-medium px-3 py-1.5 rounded-xl ${match.status === 'accepted' ? 'bg-emerald-950 text-emerald-400 border border-emerald-900' : 'bg-zinc-800 text-zinc-500'}`}>
